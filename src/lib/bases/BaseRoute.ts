...
export class BaseRoute {
  ...
  constructor(protected config: IAppConfig, protected dbUtil: DBUtil) {
  ...
  protected authenticateRequest(ctx: RouterContext): boolean {
    return (
      ctx.request.body.apiToken === this.config.apiToken ||
      ctx.request.query.apiToken === this.config.apiToken ||
      ctx.request.headers.apiToken === this.config.apiToken
    )
  }
